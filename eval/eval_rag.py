...
# Ollama 
def call_ollama(messages: list[dict], model: str,
                max_tokens: int = 512, temp: float = 0.0) -> str | None:
    try:
        resp = requests.post(
...
def parse_string_list(raw: str) -> list[str] | None:
    """
...
def parse_yes_no_list(raw: str) -> list[str] | None:
...
def faithfulness(answer: str, context: str, model: str) -> tuple[float | None, dict]:
    """
    ...
    Returns: (score, details_dict) where details_dict contains raw responses
    """
    ...
    raw1 = call_ollama(
        [{"role": "user",
          "content": _CLAIM_EXTRACT.format(answer=answer[:2000])}],
        model=model, max_tokens=400, temp=0.0,
    )
    claims = parse_string_list(raw1)
    if not claims:
    ...
    raw2 = call_ollama(
        [{"role": "user",
          "content": _CLAIM_VERIFY.format(
              context=context[:3000], claims=numbered)}],
        model=model, max_tokens=80, temp=0.0,
    )
    verdicts = parse_yes_no_list(raw2)
    if not verdicts:
        log.debug("  faith: could not parse verdicts from: %s", repr((raw2 or "")[:100]))
...
def relevancy(question: str, answer: str,
              embed_model: SentenceTransformer,
              model: str, n: int = 3) -> tuple[float | None, dict]:
    """
    Relevancy = mean cosine_similarity(generated_questions, original_question)

    Step 1: LLM generates N questions the answer addresses
    Step 2: Embed all + original query
    Step 3: Mean dot product (embeddings are normalised)
    
    Returns: (score, details_dict) where details_dict contains generated questions and similarities
    """
    if not answer:
        return None, {}
    ...
    raw = call_ollama(
        [{"role": "user",
          "content": _QUESTION_GEN.format(answer=answer[:2000], n=n)}],
        model=model, max_tokens=200, temp=0.3,
    )
    gen_qs = parse_string_list(raw)
    if not gen_qs:
...
# Main eval loop 
def run_eval(queries, retriever, system_prompt, gen_model, judge_model,
    ...
    for i, q in enumerate(queries, 1):
        ...
        log.info("  Retrieved: %s (%.2fs)",
                 ", ".join(f"{r}×{n}" for r, n in repo_counts.items()), ret_t)
        ...
        if run_judge and chunks and embed_model is not None:
            faith_val, faith_details = faithfulness(answer, context, model=judge_model)
            relev_val, relev_details = relevancy(query, answer, embed_model, model=judge_model)
